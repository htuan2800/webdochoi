...
function watchStatistics() {
    ...
    if (selectTimeElement.value === "1") { // Theo tháng
        headerRow.innerHTML = `
            ...
            <th class="text-center align-middle">Số đơn nhập</th>
            <th class="text-center align-middle">Số sản phẩm đã nhập</th>
    ...
    // jQuery Ajax request
    $.ajax({
        url: 'ajax/statistics_import.php',
        type: 'POST',
        data:{
            ...
            try {
                data.chart_data.forEach(item => {
                    ...
                    row.innerHTML = `
                        ...
                        <td class="text-center">${item.total_imports}</td>
                        <td class="text-center">${item.total_products}</td>
                        <td class="text-center">${Number(item.total_cost).toLocaleString('vi-VN')} đ</td>
                    `;
                ...
                const end = document.createElement('tr');
                    ...
                    <td class="text-center">${data.totalImport}</td>
                    <td class="text-center">${data.totalProduct}</td>
                    <td class="text-center">${data.totalPrice.toLocaleString('vi-VN')} đ</td>
                `;
...
function exportToPDF() {
    ...
    title.innerHTML = `BÁO CÁO NHẬP KHO`;
